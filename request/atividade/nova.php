...
if ($controlador->setUser($email)) {
    if (!$_SESSION['ehAluno']) {
        ...
        if (($lastId = $turma->novaAtividade($titulo, $texto, $data))) {
            $response = array(
                "resultado" => true,
                "resposta" => "Sucesso ao criar nova Atividade",
                "lastId" => $lastId,
                "log" => log::TYPE_NORMAL,
                "debug" => array("id_atividade" => $lastId)
            );
        } else {
            $response = array(
                "resultado" => false,
                ...
                "log" => log::TYPE_ERROR,
                "debug" => array("titulo" => $titulo, "texto" => $texto, "data" => $data)
            );
        }
    } else {
        $response = array(
