...
if ($isAluno) {
    ...
    if ($idturmatopico !== null) {
        foreach ($turmas as $i => $turma) {
            if ((int) $turma->getIdturma() === (int) $idturmatopico) {
                $idturmatopico = $i;
                break;
            }
        }
...
if ((int) $topico->getIduser() !== (int) $controlador->getUserId()) {
    header('./profile.php');
}

//include './includes/spellChecker.php';
?>
<html lang="pt-BR">
    <head>
    ...
    <body>
        <div class='topbar'>
            ...
            <div class="subtitle"><h1>Editar Tópico</h1></div>
            <div id='gotohub' onclick="toggleHUB();">
                <p>HUB</p>
        ...
        <div class="area">
            <form class="pure-form" onsubmit="publicar(event)">
                ...
                <input id="titulo" autocomplete="off" required="required" placeholder="Máximo 100 caracteres" value="<?= $topico->getTitulo() ?>"  maxlength="100" name='titulo' type="text">
                <label for="texto">Texto</label>
                <textarea id="texto" name="texto" required="required" ><?= $topico->getTexto() ?></textarea>
                <p>
                ...
                <button class="pure-button pure-button-primary" id='post'>Alterar</button>
                <button class="pure-button" onclick="window.location = './topicoview.php?id=<?= $topico->getId(); ?>';"  type='button'>Cancelar</button>
            </form>
        </div>
        ...
        <script async>
            publicar = function (event) {
                ...
                post("./request/topicos/updateTopico.php", function () {
                    redirect("./topicoview.php", {id: <?= $id ?>});
                }, {
                    titulo: $("#titulo").val(),
                    texto: $("#texto").val(),
            ...
            $("#turmaselector").val('<?= ($idturmatopico !== null ? $idturmatopico : 'null') ?>');
