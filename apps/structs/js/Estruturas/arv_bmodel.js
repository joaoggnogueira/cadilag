...
function TREEB() {
    ...
    this.init = function(disc){
        ...
        this.qtd = 0;
        this.disc = disc;
        Element.bind(this,this.disc.id);
        while(i<ORDEM)
        {
            this.campos[i] = null;
            this.filhos[i] = null;
    ...
    this.SplitDown = function(No,ListaAnimation){
        ...
        ListaAnimation.add(new PackActions([new arrumarArvore(collection),new lancarTooltip("Split para baixo<br>&#8595;","green",No.disc,50)]));
    ...
    this.SplitUp = function(No,animation){
        ...
        var j=Pai.qtd;
        animation.add(new PackActions([new arrumarArvore(collection),new lancarTooltip("&#8593;<br>Sobe um nó","red",No.disc,-70)]));
...
//        disc.setInicial(true);
        ...
        $(collection.disc.d).append("<span class='initial'>Raiz</span>");
    },
    ...
    window.Cadilag = {
        ...
        elementToString: function(id){
            ...
            if (element !== undefined) {
                var result = {}; 
                if(element.pai) {
                    var nextId = (element.disc.id * 1).toString(16).toUpperCase();
                    result.pai = '*' + (nextId.substring(nextId.length - 5));
                } else {
                    result.pai = "nulo";
                }
                
                result.qtd = element.qtd;
                result.qtdfilhos = element.qtdfilhos;

                var arrayCampos = false;
                for(var i = 0; i < ORDEM; i++){
                    var info = (element.campos[i]?element.campos[i]:"nulo");
                    if(!arrayCampos) {
                        arrayCampos = "["+info;
                    } else {
                        arrayCampos += ", "+info;
                ...
                result.info = arrayCampos;
                
                var arrayFilhos = false;
                for(var i = 0; i <= ORDEM; i++){
                    var id;
                    if(element.filhos[i]) {
                        var nextId = (element.filhos[i].disc.id * 1).toString(16).toUpperCase();
                        id = '*' + (nextId.substring(nextId.length - 5));
                    } else {
                        id = "nulo"; 
                    }
