...
(function () {
    ...
    addCallStack = function(info,leitor,line,name){
        var arvoreNo = {resumo:"<i>nulo</i>",tipo: "EstruturaArvore"};
        if(leitor.ref.ref !== null) {
            var id = (leitor.ref.id * 1).toString(16).toUpperCase();
            id = '*' + (id.substring(id.length - 5));

            var esqId;
            if (leitor.ref.esq.ref.ref !== null) {
                var nextId = (leitor.ref.esq.ref.id * 1).toString(16).toUpperCase();
                esqId = '*' + (nextId.substring(nextId.length - 5));
            } else {
                esqId = 'nulo';
            }
            ...
            if (leitor.ref.dir.ref.ref !== null) {
                var nextId = (leitor.ref.dir.ref.id * 1).toString(16).toUpperCase();
                dirId = '*' + (nextId.substring(nextId.length - 5));
            } else {
                dirId = 'nulo';
            }
            arvoreNo = {
                resumo: "<i>"+id+"</i>",
                tipo: "EstruturaArvore",
                info: leitor.ref.value,
                direita: "<i>"+dirId+"</i>",
                esquerda: "<i>"+esqId+"</i>",
                hover: leitor.ref.id
            };
        ...
        UI.updatePilhaDeChamadas();
    },

    addCallStackInsere = function(info,leitor,line){
        addCallStack(info,leitor,line,"Insere");
    },
    
    addCallStackBusca = function(info,leitor,line){
        addCallStack(info,leitor,line,"Busca");
    },
    
    addCallStackRemove = function(info,leitor,line){
        addCallStack(info,leitor,line,"Remove");
    },
    ...
    animationAdd = function (info) {
        ...
        UI.registroDePilhaDeChamadas();
        StepRecord.add("Estado Inicial");
        var pai = null;
        var raiz = collection.raiz;
        addCallStackInsere(info,raiz,"Insere("+info.valor+",Raiz)");
        ...
        var animateArrowLeft = function(no,text,i){
            timeout( function(){
                Tooltip.show(no.ref.d.id, "<b>"+text+"</b>", "#F44", 2000);
            }, i*2000);
        };
        var animateArrowRight = function(no,text,i){
            timeout( function(){
                Tooltip.show(no.ref.d.id, "<b>"+text+"</b>", "#44F", 2000);
            }, i*2000);
        };        
        var animateArrowEqual = function(no,text,i){
            timeout( function(){
                Tooltip.show(no.ref.d.id, "<b>"+text+"</b>", "#444", 2000);
            }, i*2000);
        };        
        ...
        while (raiz.ref.ref !== null) {
            ...
            if (parseInt(info.valor) < parseInt(raiz.ref.value)) {
                animateArrowLeft(raiz,info.valor+" < "+raiz.ref.value,i);
                timeout( function(){
                ...
                highlighter.lineGreen(3, 1200 + 2000 * i,function(data){
                    addCallStackInsere(info,data.raiz,data.line);
                },{raiz:raiz});
            } else if (parseInt(info.valor) > parseInt(raiz.ref.value)) {
                animateArrowRight(raiz,info.valor+" > "+raiz.ref.value,i);
                timeout( function() {
                ...
                highlighter.lineGreen(5, 1500 + 2000 * i,function(data){
                    addCallStackInsere(info,data.raiz,data.line);
                },{raiz:raiz});
            } else {
                animateArrowEqual(raiz,info.valor+" == "+raiz.ref.value,i);
        ...
        highlighter.lines(12, 14, 2000 * (i + 1));
    ...
    animationSearch = function (info) {
        ...
        var raiz = collection.raiz;
        UI.registroDePilhaDeChamadas();
        addCallStackBusca(info,raiz,"Busca("+info.valor+",Raiz)");
        ...
        while (raiz.ref.ref != null) {
            ...
            if (parseInt(info.valor) < parseInt(raiz.ref.value)) {
                ...
                raiz = raiz.ref.esq;
                highlighter.lineGreen(3, 1200 + atual,function(data){
                    addCallStackBusca(info,data.raiz,data.line);
                },{raiz:raiz});
            } else if (parseInt(info.valor) > parseInt(raiz.ref.value)) {
                timeout( function(){
                    StepRecord.add("Busca continua para subárvore da direita");
                ...
                highlighter.lineGreen(5, 1000 + atual,function(data){
                    addCallStackBusca(info,data.raiz,data.line);
                },{raiz:raiz});
            } else {
                timeout( function(){
    ...
    animationRemove = function (info) {
        ...
        var raiz = collection.raiz;
        UI.registroDePilhaDeChamadas();
        addCallStackRemove(info,raiz,"Remove("+info.valor+",Raiz)");
        var pai = null;
        ...
        while (raiz.ref.ref != null) {
            ...
            if (parseInt(info.valor) < parseInt(raiz.ref.value)) {
                ...
                highlighter.lineGreen(5,(j+1)*2000,function(data){
                    addCallStackRemove(info,data.raiz,data.line);
                },{raiz:raiz}); 
                timeout( function(){
                   StepRecord.add("Busca continua para subárvore da esquerda");
                },(j+2)*2000);
                ...
                highlighter.clear((j+1)*2000);
                highlighter.lineGreen(3,(j+1)*2000,function(data){
                    addCallStackRemove(info,data.raiz,data.line);
                },{raiz:raiz});
                timeout( function(){
                   StepRecord.add("Busca continua para subárvore da direita");
