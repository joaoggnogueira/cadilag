...
(function () {
    ...
    animationRemove = function (p) {
        ...
        if(collection.topos[p]!==collection.topoInicial(p)){
            ...
            Plumb.showMessage("Pilha vazia",6000);
            highlighter.lineRed(0,10);
        ...
        newStruct: function(){
            ...
            if(size!=="" && total!=="") {
                ...
                for (var i = 0; i <= total; i++) {
                    ...
                    params += collection.inicios[i];
                }
                UI.setParameters({inicios:"{"+params+"}"});
                draw();
                initPointer();    
                UI.unlock();
