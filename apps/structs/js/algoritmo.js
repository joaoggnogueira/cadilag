...
window.AlgServer = {
    ...
    }, get : function(line,alg) {
        
        if(alg === VK.INSERCAO) {
            return this.algoritmos[this.selected].addlines[line].valueAtual;
        } else if(alg === VK.REMOCAO) {
            return this.algoritmos[this.selected].remlines[line].valueAtual;            
        } else if(alg === VK.BUSCA){
            return this.algoritmos[this.selected].searchlines[line].valueAtual;            
        }
...
function HighlighterAlgoritmo(algoritmo) {
    ...
    // Somente para excecução de desvio de fluxo entre rotinas 
    this.lineGreen = function (line, time, callback, datacallback) {
        var vk = this.vk;
        timeout(function () {
            var obj = algoritmoSelecionado.find("#l" + line);
            obj.addClass('call_line');
            if(callback) {
                datacallback.line = AlgServer.get(line,vk);
