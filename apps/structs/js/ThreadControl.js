...
function stopAll() {
    ...
    for (var key in threads_ativas) {
        ...
        t.delay = t.started - Date.now() + t.delay;
        clearTimeout(t.t);
    }
    ...
    $(".element").each(function(key, obj){
        ...
        if (queue && queue.length !== 0) {
            queue[0].time = queue[0].started - Date.now() + queue[0].time;
            $(obj).stop(true, false);
        }
    });

    if (currentInterval !== null && !dangerInterval) {
        currentInterval.delay = currentInterval.started - Date.now() + currentInterval.delay;
