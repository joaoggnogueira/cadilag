...
CREATE PROCEDURE `novoTopico`(_idUser INT,_topAssunto VARCHAR(100),_textTopico LONGTEXT,_idturma INT,_particular BOOLEAN)
BEGIN
	IF exists (SELECT * FROM Usuario WHERE Usuario.usu_id=_idUser) THEN
		BEGIN
			INSERT INTO Topico(usu_id,top_assunto,topico_texto, data, hora,Turma_idTurma,visible_only_by_class) VALUES (_idUser,_topAssunto,_textTopico, current_date(), current_time(),_idturma,_particular);
